...
def filter_date(outcome, state, start_date, end_date):
    ...
    total_success = df[df["Outcome"] == "Success"].groupby("Date")[
        "Outcome"].count()
    # 3- Total number of failures per date
    total_failure = df[df["Outcome"] == "Failure"].groupby("Date")[
        "Outcome"].count()
    ...
    success_bystate = (
        df[df["Outcome"] == "Success"].groupby(
            "State")["Outcome"].count().reset_index()
    )
    # print(success_bystate)

    # Total number of failures by state
    failure_bystate = (
        df[df["Outcome"] == "Failure"].groupby(
            "State")["Outcome"].count().reset_index()
    )
    ...
    totsuccess = df[df["Outcome"] == "Success"].groupby("State")[
        "Outcome"].count()
...
def q1():
    ...
    fig_total.add_scatter(x=dfsd.index, y=dfsd.values,
